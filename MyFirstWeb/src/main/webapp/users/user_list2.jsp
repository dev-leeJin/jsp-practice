...
<%
	...
	// Value Object는 클래스이기 때문에 src/main/java에 클래스를 선언 및 정의합니다.
	...
	// 1. DB접속 변수 생성
		...
		// ArrayList<UserVO> 자료형을 이용해 한 줄 한 줄 단위로 여러 row를 저장합니다.(가변적인ArrayList특성)
	...
	// 3. SELECT * FROM userinfo 실행 및  ResultSet에 저장
			...
			// 테이블에 저장된 데이터를 자바로 옯기기 위해서 UserVO를 선언했습니다.
			...
			// .close()이전에 DB에 있던 데이터를 자바로 옮겨야 합니다.
			con.close();
			pstmt.close();
			rs.close();
		}
		...
		*/
		...
		List<UserVO> userList= dao.getAllUserList(); // DB내 전체 데이터를 우측에 배달받아 좌변에 저장(전달) 
...
<body>
	...
	<% for(UserVO user :userList){ %> <!-- UserVO의 집합이기 때문에 userList를 하나하나 가져오기 위해
											UserVO 타입의 변수명 user로 받아온다.  -->
